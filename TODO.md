...
- [ ] `git init`, add a proper `.gitignore` (node_modules, dist, __pycache__, .pytest_cache, .env, the 11 MB raw GOV.UK CSV — keep only the build script + derived `sponsors.csv` if size allows, or document the download step).
- [ ] First commit: current state, with a clear message. Then commit **in meaningful units** going forward (one feature/fix per commit) so the history shows process.
- [ ] Create public GitHub repo `visajobfinder` (or similar). Push.
- [ ] Verify `.env` and any API keys are NOT in history before pushing.
